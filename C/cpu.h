...
#define LDA_ABS(addr)   do { A = mem[(addr)]; SET_ZN(A); } while(0)
#define LDA_ABSX(addr)  do { A = mem[((addr) + X) & 0xFFFF]; SET_ZN(A); } while(0)
#define LDA_ABSY(addr)  do { A = mem[((addr) + Y) & 0xFFFF]; SET_ZN(A); } while(0)
#define LDA_INDY(addr)  do { \
    uint16_t ptr = mem[(addr) & 0xFF] | (mem[((addr) + 1) & 0xFF] << 8); \
    A = mem[(ptr + Y) & 0xFFFF]; SET_ZN(A); \
} while(0)
...
#define LDX_ABS(addr)   do { X = mem[(addr)]; SET_ZN(X); } while(0)
#define LDX_ABSY(addr)  do { X = mem[((addr) + Y) & 0xFFFF]; SET_ZN(X); } while(0)
...
#define LDY_ABS(addr)   do { Y = mem[(addr)]; SET_ZN(Y); } while(0)
#define LDY_ABSX(addr)  do { Y = mem[((addr) + X) & 0xFFFF]; SET_ZN(Y); } while(0)

// STA
#define STA_ZP(addr)    do { mem[(addr) & 0xFF] = A; } while(0)
#define STA_ZPX(addr)   do { mem[((addr) + X) & 0xFF] = A; } while(0)
#define STA_ABS(addr)   do { mem[(addr)] = A; } while(0)
#define STA_ABSX(addr)  do { mem[((addr) + X) & 0xFFFF] = A; } while(0)
#define STA_ABSY(addr)  do { mem[((addr) + Y) & 0xFFFF] = A; } while(0)
...
#define ADC_ABS(addr) _ADC(mem[addr])
...
#define SBC_ABS(addr) _SBC(mem[addr])
...
#define CMP_ABS(addr) _CMP(mem[addr])
...
#define CPX_ABS(addr) _CPX(mem[addr])
...
#define INC_ABS(addr)  do { mem[(addr)]++; SET_ZN(mem[(addr)]); } while(0)
#define DEC_ZP(addr)   do { mem[(addr) & 0xFF]--; SET_ZN(mem[(addr) & 0xFF]); } while(0)
#define DEC_ABS(addr)  do { mem[(addr)]--; SET_ZN(mem[(addr)]); } while(0)
